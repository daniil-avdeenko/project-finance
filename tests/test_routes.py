...
from app.models import Project, Employee, IncomeCategory, ExpenseCategory, Transaction
...
def test_404_page(auth_client):
    ...
    assert 'Страница не найдена'.encode('utf-8') in response.data
...
def test_projects_list_authorized(auth_client):
    ...
    assert response.status_code == 200
    assert 'Проекты'.encode('utf-8') in response.data
...
def test_transactions_list_authorized(auth_client):
    ...
    assert response.status_code == 200
    assert 'Транзакции'.encode('utf-8') in response.data
...
def test_employees_list_authorized(auth_client):
    ...
    assert response.status_code == 200
    assert 'Сотрудники'.encode('utf-8') in response.data
...
def test_edit_project(auth_client, app):
    ...
    response = auth_client.post(f'/projects/{project_id}/edit', data={
        'name': 'Новое название',
...
def test_edit_employee_with_projects(auth_client, app):
    ...
    response = auth_client.post(f'/employees/{emp_id}/edit', data={
...
def test_edit_transaction(auth_client, app):
    ...
    response = auth_client.post(f'/transactions/{t_id}/edit', data={
        'type': 'income',
        'project_id': p_id,
        'category_id': c_id,
        'amount': '5000',
...
def test_create_income_category_via_route(auth_client, app):
    ...
    response = auth_client.post('/income-categories/create', data={
...
def test_edit_income_category(auth_client, app):
    ...
    response = auth_client.post(f'/income-categories/{c_id}/edit', data={
        'name': 'Новое название дохода'
    }, follow_redirects=True)
    assert response.status_code == 200
...
def test_create_expense_category_via_route(auth_client, app):
    ...
    response = auth_client.post('/expense-categories/create', data={
        'name': 'Новая категория расхода'
    }, follow_redirects=True)
    assert response.status_code == 200
...
def test_edit_expense_category(auth_client, app):
    ...
    response = auth_client.post(f'/expense-categories/{c_id}/edit', data={
        'name': 'Новое название'
    }, follow_redirects=True)
    assert response.status_code == 200
...
def test_transactions_filter_by_type(auth_client, app):
    ...
    with app.app_context():
        ...
        _db.session.add(Transaction(project_id=project.id, type='income',
                                    category_id=cat.id, amount=1000))
        _db.session.add(Transaction(project_id=project.id, type='expense',
                                    category_id=expense_cat.id, amount=2000))
        _db.session.commit()
