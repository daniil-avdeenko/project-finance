...
from app import db
from app.models import Transaction, Project, IncomeCategory, ExpenseCategory
...
def transactions_list():
    ...
    if date_to:
        try:
            date_to_obj = datetime.strptime(date_to, '%Y-%m-%d').replace(tzinfo=timezone.utc, hour=23, minute=59, second=59)
    ...
    if request.method == 'POST':
        ...
        if form.validate_on_submit():
            ...
            transaction = Transaction(
                ...
                date=form.date.data or datetime.now(timezone.utc)
            )
            try:
...
def export_transactions_csv():
    ...
    for t in transactions:
        category_name = income_categories.get(t.category_id, '') if t.type == 'income' else expense_categories.get(t.category_id, '')
